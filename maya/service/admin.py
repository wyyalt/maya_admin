...
from maya.utils.pagination import Page
...
class FilterList(object):
    ...
    def __init__(self, option, data_list,request,extend=False):
        self.option = option
    ...
    def __iter__(self):
        ...
        for obj in self.data_list:
            ...
            if self.extend:
                val = obj[0] if obj[0] else 'Null'
                text = obj[1] if obj[1] else '空值'
            else:
            ...
            exist = False
            if val in param_dict.getlist(self.option.name):
                exist = True

            if self.option.is_multi:
                if exist:
                    param_dict.getlist(self.option.name).remove(val)
                else:
                    param_dict.appendlist(self.option.name, val)
            else:
                param_dict[self.option.name] = val
            url = "{0}?{1}".format(base_url, param_dict.urlencode())
            tpl_str = tpl.format(url, 'active' if exist else '', text)
            yield mark_safe(tpl_str)
        yield mark_safe("</div>")
...
class ChangeList(object):
    def __init__(self, request, maya_model, list_display, result_list, model_cls, list_filter, actions):
        self.request = request
        self.list_display = list_display
        self.list_filter = list_filter

        self.model_cls = model_cls
        self.maya_model = maya_model
        ...
        self.pager = Page(self.request.GET.get('page'), all_count, base_url=self.maya_model.changelist_url(),
    ...
    def add_btn(self):
        ...
        """
        add_url = reverse(
            '%s:%s_%s_add' % (self.maya_model.site.namespace, self.maya_model.app_label, self.maya_model.model_name))
...
class BaseMayaModel(object):
    ...
    def changelist_view(self, request):
        ...
        return TemplateResponse(request, self.change_list_template or [
            'maya/%s/%s/change_list.html' % (self.app_label, self.model_name),
            'maya/%s/change_list.html' % self.app_label,
            'maya/change_list.html'
        ], context)
    ...
    def add_view(self, request):
        ...
        if request.method == 'GET':
            ...
            if form.is_valid():
                ...
                if popup_id:
                    ...
                    return SimpleTemplateResponse('maya/popup_response.html',
        ...
        return TemplateResponse(request, self.add_form_template or [
            'maya/%s/%s/add.html' % (self.app_label, self.model_name),
            'maya/%s/add.html' % self.app_label,
            'maya/add.html'
        ], context)
    ...
    def change_view(self, request, pk):
        ...
        return TemplateResponse(request, self.change_form_template or [
            'maya/%s/%s/change.html' % (self.app_label, self.model_name),
            'maya/%s/change.html' % self.app_label,
            'maya/change.html'
        ], context)
    ...
    def detail_view(self, request, pk):
        ...
        return TemplateResponse(request, self.change_form_template or [
            'maya/%s/%s/detail.html' % (self.app_label, self.model_name),
            'maya/%s/detail.html' % self.app_label,
            'maya/detail.html'
        ], context)


class MayaSite(object):
    def __init__(self, app_name='maya', namespace='maya'):
        self.app_name = app_name
        self.namespace = namespace
        self._registry = {}

    def register(self, model_class, maya_model_class=BaseMayaModel):
        self._registry[model_class] = maya_model_class(model_class, self)
    ...
    def get_urls(self):
        ...
        for model_class, maya_model_obj in self._registry.items():
            urlpatterns += [
                url(r'^%s/%s/' % (model_class._meta.app_label, model_class._meta.model_name),
                    include(maya_model_obj.urls))
            ]
        return urlpatterns
    ...
    def index(self, request):
        ...
        :return: 
        """
        return render(request, 'maya/index.html')
