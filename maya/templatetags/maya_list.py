...
def result_header_list(change_list):
    ...
    if change_list.list_display == "__str__":
        yield change_list.maya_model.model_name
    else:
        for name in change_list.list_display:
            yield name(change_list.maya_model, is_header=True) if isinstance(name,
...
def result_body_list(change_list):
    ...
    for row in change_list.result_list:
        if change_list.list_display == "__str__":
            ...
            yield [name(change_list.maya_model, model_obj=row) if isinstance(name, FunctionType) else getattr(row, name) for
                   name
                   in change_list.list_display]


@register.inclusion_tag('maya/change_list_results.html')
...
@register.inclusion_tag('maya/change_list_action.html')
def show_action(change_list):
    return {'actions': ((item.__name__, item.text) for item in change_list.actions)}
